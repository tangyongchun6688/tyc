		...
		if (limitOffset.length == 2) {
			...
			final BoundSql newBoundSql = copyFromBoundSql(mappedStatement, boundSql, getLimitSql(originalSql,offset,end-1)); 
			 ...
			 if (parameter instanceof Map){
				 ...
				 map = objectToMap(parameter);
			 }
				 
			 //Map map = objectToMap(parameter);
			// Map<String, String> map = (Map<String, String>)parameter;
			 map.put("rowCount", totpage + "");
			 //每页显示条数
			 int pageSize = limitOffset[1];
			 
			 if(totpage%pageSize > 0){
				 map.put("pageCount", ( totpage/pageSize + 1 ) + "");
			 }else{
				 map.put("pageCount", totpage/pageSize + "");
			 }
	...
	private int[] getLimitOffset(Object o) {
		...
		try {
			if (o instanceof Object) {
				...
				//Map map = (Map)o;
				if (map.containsKey("pageNow")&&map.containsKey("pageSize")) {//
					Object offset = map.get("pageNow");
					Object limit = map.get("pageSize");
