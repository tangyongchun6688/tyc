...
//        }
     ...
     * @return
     */
    public static boolean sendToUserClient(String message,String usernoRand){
    	boolean flag =false;
    	try {
    		if (WebSocketServer.webSocketSet.get(usernoRand) != null){
    			Session session = WebSocketServer.webSocketSet.get(usernoRand).getSession();
     ...
     * @return
     */
    public static boolean sendAllClient(String message,String usernoRand){
    	boolean flag =false;
    	try {
    		//遍历HashMap
    		for (String key : WebSocketServer.webSocketSet.keySet()) {
    			//判断接收用户是否是当前发消息的用户
//                if (!usernoRand.equals(key)) {
                	Session session = WebSocketServer.webSocketSet.get(key).getSession();
        			session.getBasicRemote().sendText(message);
//                }
    		}
    		flag = true;
